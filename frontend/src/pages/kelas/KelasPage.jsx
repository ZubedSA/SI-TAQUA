...
const KelasPage = () => {
    ...
    const { data: queryData, isLoading: loading, refetch: fetchKelas } = useKelasList()
    const kelasList = queryData?.kelasList || []
    const santriCounts = queryData?.santriCounts || {}
    ...
    const fetchCurrentSemester = async () => {
        try {
            const { data, error } = await supabase.from('semester').select('id').eq('is_active', true).single()
            if (error) {
                console.warn('No active semester found or error:', error.message)
                return
            }
    ...
    const openDetail = (kelas) => {
        fetchSantriByKelas(kelas);
    }

    const openDelete = (kelas, e) => {
        if (e) e.stopPropagation();
        confirmDelete(kelas, e);
    }
    ...
    const openAddSantriModal = async (kelas, e) => {
        if (e) e.stopPropagation();
        setSelectedKelas(kelas)
    ...
    const confirmRemoveSantri = (santriId, e) => {
        if (e) e.stopPropagation();
    ...
    return (
        <div className="space-y-6">
            <PageHeader
                ...
                actions={
                    <div className="flex gap-2">
                        <Button variant="secondary" onClick={fetchKelas}>
                            <RefreshCw size={18} />
                        </Button>
                        {canEdit && (
                                ...
                                <div className="mt-auto flex items-center justify-between pt-4 border-t border-gray-100">
                                    ...
                                    {canEdit && (
                                        <Button
                                            variant="secondary"
                                            size="sm"
                                            onClick={(e) => openAddSantriModal(kelas, e)}
                                            className="!px-3 !py-1"
                                        >
                                            <Plus size={14} /> Santri
                                        </Button>
                ...
                <div className="modal-overlay visible opacity-100" style={{ zIndex: 999999, display: 'flex', visibility: 'visible', opacity: 1 }}>
                    <div className="modal-box w-full max-w-3xl flex flex-col">
                        <div className="flex items-center justify-between px-6 py-4 border-b border-gray-200 shrink-0">
                            ...
                            <button type="button" className="text-gray-400 hover:text-gray-600 transition-colors" onClick={() => setShowSantriModal(false)}>
                                <X size={24} />
                            </button>
                        </div>
                        <div className="p-0 overflow-auto">
                            {loadingSantri ? (
                                ...
                                <div className="text-center py-12 px-6">
                                    ...
                                    {canEdit && (
                                        <Button type="button" onClick={(e) => openAddSantriModal(selectedKelas, e)}>
                        ...
                        <div className="px-6 py-4 bg-gray-50 border-t border-gray-200 flex justify-end gap-3 shrink-0">
                            ...
                            <Button type="button" variant="secondary" onClick={() => setShowSantriModal(false)}>
                                Tutup
                            </Button>
                        </div>
                    </div>
            ...
            {showAddSantriModal && createPortal(
                <div className="modal-overlay visible opacity-100" style={{ zIndex: 999999, display: 'flex', visibility: 'visible', opacity: 1 }}>
                    <div className="modal-box w-full max-w-2xl flex flex-col">
                        <div className="flex items-center justify-between px-6 py-4 border-b border-gray-200 shrink-0">
                            <h3 className="text-lg font-semibold text-gray-900 flex items-center gap-2">
                                <UserPlus size={20} className="text-primary-600" />
                                Tambah Santri ke {selectedKelas?.nama}
                            </h3>
                            <button type="button" className="text-gray-400 hover:text-gray-600 transition-colors" onClick={() => setShowAddSantriModal(false)}>
                                <X size={24} />
                            </button>
                        </div>
                        ...
                        <div className="px-6 py-4 bg-gray-50 border-t border-gray-200 flex justify-end gap-3 shrink-0">
                            <Button type="button" variant="secondary" onClick={() => setShowAddSantriModal(false)}>
                                Batal
                            </Button>
                            <Button
                                type="button"
                ...
                <div className="modal-overlay visible opacity-100" style={{ zIndex: 999999, display: 'flex', visibility: 'visible', opacity: 1 }}>
                    <div className="modal-box w-full max-w-lg">
                        <div className="flex items-center justify-between px-6 py-4 border-b border-gray-200">
                            ...
                            <button type="button" className="text-gray-400 hover:text-gray-600 transition-colors" onClick={() => { setShowModal(false); setEditData(null) }}>
                                <X size={24} />
                            </button>
                        </div>
