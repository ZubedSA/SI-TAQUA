...
const GuruForm = () => {
    ...
    const canEdit = (isAdmin() || isAdminAkademik() || userProfile?.role === 'admin' || hasRole('admin')) && isEdit
    ...
    const executeSave = async () => {
        ...
        try {
            ...
            if (isEdit) {
                ...
                // SYNC TO USER PROFILES & AUTH (Jika email berubah)
                if (formData.email && formData.email !== originalEmail) {
                    console.log('🔄 Email changed, syncing to auth and profiles...', { from: originalEmail, to: formData.email })
                    // 1. Cari profile user berdasarkan email lama
                    ...
                    if (profile) {
                        // 2. Gunakan RPC untuk update email auth & profil
                        const { error: rpcError } = await supabase.rpc('admin_update_user_email', {
                            target_user_id: profile.user_id,
                            new_email: formData.email,
                            new_username: profile.username,
                            new_full_name: formData.nama, // Gunakan nama baru dari form
                            new_role: profile.role,
                            new_roles: profile.roles || [],
                            new_active_role: profile.active_role,
                            new_phone: formData.no_telp || profile.phone
                        })
                        if (rpcError) console.warn('Sync to auth failed:', rpcError.message)
                    }
                } else if (formData.email) {
                    // Update nama saja jika email tetap
                    await supabase
                        .from('user_profiles')
                ...
                setOriginalEmail(formData.email) // Update original email after success
            } else {
                const { error } = await supabase
                    .from('guru')
                    .insert([payload])
