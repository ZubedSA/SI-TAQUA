    ...
    const handleUpdateEmail = async (e) => {
        ...
        try {
            ...
            if (rpcResult.success) {
                // [SYNC] Update email di tabel guru jika user adalah pengajar/musyrif
                const roles = userProfile?.roles || []
                if (roles.includes('guru') || roles.includes('musyrif')) {
                    console.log('🔄 Syncing email change to guru table...')
                    await supabase
                        .from('guru')
                        .update({ email: email })
                        .eq('email', user.email) // Gunakan email lama dari auth session
                }
