            ...
            <div className="bg-white rounded-2xl shadow-sm border border-slate-200 overflow-hidden">
                ...
                <div className="w-full">
                    <ResponsiveTable
                        columns={[
                            {
                                ...
                                render: (row) => (
                                    <div className="flex items-center gap-4">
                                        ...
                                        <div>
                                            <div
                                                onClick={() => navigate(`/admin/ota/${row.id}`)}
                                                className="font-bold text-slate-700 hover:text-emerald-600 cursor-pointer transition-colors"
                                            >
                                                {row.nama}
                                            </div>
                                            <div className="text-xs text-slate-400 font-mono mt-0.5">ID: {row.id.slice(0, 8)}</div>
                                        </div>
                                    </div>
                                ),
                                className: 'px-6 py-4'
                            ...
                            },
                            {
                                header: 'Binaan',
                                render: (row) => (
                                    <span className={`inline-flex items-center gap-1.5 px-3 py-1 rounded-lg text-xs font-semibold border ${row.ota_santri?.[0]?.count > 0 ? 'bg-blue-50 text-blue-700 border-blue-100' : 'bg-slate-50 text-slate-500 border-slate-100'}`}>
                                        <User size={12} />
                                        {row.ota_santri?.[0]?.count || 0} Santri
                                    </span>
                                ),
                                className: 'px-6 py-4',
                                hideOnMobile: true
                            },
                            {
                                header: 'Status',
                                render: (row) => (
                                    <div className={`inline-flex items-center gap-2 px-3 py-1.5 rounded-full text-xs font-semibold border ${row.status ? 'bg-emerald-50 text-emerald-700 border-emerald-200' : 'bg-rose-50 text-rose-700 border-rose-200'}`}>
                                        <span className={`w-2 h-2 rounded-full ${row.status ? 'bg-emerald-500 animate-pulse' : 'bg-rose-500'}`}></span>
                            ...
                            },
                            {
                                header: 'Aksi',
                                render: (row) => (
                                    <div className="flex justify-end gap-2 opacity-80 group-hover:opacity-100 transition-opacity" onClick={(e) => e.stopPropagation()}>
                                        <button
                                            onClick={() => navigate(`/admin/ota/${row.id}`)}
                                        ...
                                        <button
                                            onClick={() => navigate(`/admin/ota/${row.id}/edit`)}
                        ...
                        data={filteredOtas}
                        loading={loading}
                        onRowClick={(row) => navigate(`/admin/ota/${row.id}`)}
                        emptyState={
                            ...
                            <div className="flex justify-between items-start w-full">
                                <div className="flex items-center gap-3">
                                    ...
                                    <div>
                                        ...
                                        <div className="text-xs text-slate-400 font-mono">ID: {row.id.slice(0, 8)}</div>
                                    </div>
                                </div>
                                <div className={`px-2 py-1 rounded-md text-[10px] font-bold uppercase tracking-wide border ${row.status ? 'bg-emerald-50 text-emerald-700 border-emerald-100' : 'bg-rose-50 text-rose-700 border-rose-100'}`}>
                                    {row.status ? 'Aktif' : 'Non-Aktif'}
                                </div>
                            </div>
                        )}
                                ...
                                <div className="flex items-center justify-between pt-3 border-t border-slate-100" onClick={(e) => e.stopPropagation()}>
                                    <div className="flex gap-2">
                                        ...
                                        <span className="flex items-center gap-1 px-2 py-1 bg-blue-50 text-blue-700 rounded text-xs font-medium border border-blue-100">
                                            <User size={10} /> {row.ota_santri?.[0]?.count || 0}
                                        </span>
                                    </div>
