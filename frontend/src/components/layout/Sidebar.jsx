...
const adminMenuItems = [
    ...
    { path: '/admin-absensi', icon: QrCode, label: 'Admin Absensi' },
    ...
    // Monitoring Pengurus
    {
        ...
        children: [
            ...
            { path: '/pengurus/pelanggaran/rekap', icon: FileText, label: 'Rekap Pelanggaran' },
...
const operatorMenuItems = [
    { path: '/home', icon: LayoutDashboard, label: 'Dashboard', roles: ['guru', 'bendahara', 'pengasuh', 'admin_akademik'] },
    ...
    // Data Pondok Menu dengan Nested Submenu
    {
        ...
        children: [
            { path: '/santri', icon: Users, label: 'Data Santri', roles: ['admin', 'admin_akademik'] },
            { path: '/guru', icon: GraduationCap, label: 'Data Guru', roles: ['admin', 'admin_akademik'] },
            { path: '/kelas', icon: Home, label: 'Kelas', roles: ['admin', 'admin_akademik'] },
            { path: '/mapel', icon: BookOpen, label: 'Mapel', roles: ['admin', 'admin_akademik'] },
            { path: '/jadwal', icon: Calendar, label: 'Jadwal Pelajaran', roles: ['admin', 'admin_akademik'] },
            { path: '/akademik/kalender', icon: Calendar, label: 'Kalender Akademik', roles: ['admin', 'admin_akademik'] },
            { path: '/halaqoh', icon: Circle, label: 'Halaqoh', roles: ['admin', 'admin_akademik'] },
            { path: '/semester', icon: Calendar, label: 'Semester', roles: ['admin', 'admin_akademik'] },
        ]
    },

    {
        ...
        icon: School,
        label: 'Akademik',
        roles: ['admin', 'admin_akademik', 'guru'],
        children: [
            // Agenda Mengajar (Jurnal)
            { path: '/absensi/agenda', icon: BookOpen, label: 'Agenda Mengajar', roles: ['admin', 'admin_akademik', 'guru'] },
            ...
            // Input Nilai - root menu
            {
                ...
                children: [
                    ...
                    { path: '/akademik/nilai/tahfizh/syahri', icon: Calendar, label: 'Ujian Syahri', roles: ['admin', 'admin_akademik', 'guru'] },
                    // Ujian Semester dengan submenu Tahfiziyah dan Madrasiyah
                    {
                        ...
                        roles: ['admin', 'admin_akademik', 'guru'],
                        children: [
                            { path: '/akademik/nilai/tahfizh/semester', icon: BookMarked, label: "Qur'aniyah", roles: ['admin', 'admin_akademik', 'guru'] },
                            { path: '/akademik/nilai/madros/uas', icon: BookOpen, label: 'Madrasiyah', roles: ['admin', 'admin_akademik', 'guru'] },
                        ]
                    },
                    // Perilaku & Catatan (New)
                    { path: '/akademik/nilai/perilaku', icon: PenLine, label: 'Perilaku & Catatan', roles: ['admin', 'admin_akademik', 'guru'] },
                ]
            },
            // Hafalan Menu
            {
                ...
                roles: ['admin', 'admin_akademik', 'guru'],
                children: [
                    { path: '/hafalan', icon: PenLine, label: 'Input Hafalan', roles: ['admin', 'admin_akademik', 'guru'] },
                    { path: '/hafalan?tab=rekap', icon: FileText, label: 'Rekap Hafalan', roles: ['admin', 'admin_akademik', 'guru'] },
                ]
            },
            // Laporan Menu - sesuai struktur target
            {
                ...
                children: [
                    // Laporan Nilai
                    {
                        ...
                        roles: ['admin', 'admin_akademik', 'guru'],
                        children: [
                            { path: '/laporan/ujian-syahri', icon: Calendar, label: 'Ujian Syahri', roles: ['admin', 'admin_akademik', 'guru'] },
                            { path: '/laporan/ujian-semester', icon: CalendarCheck, label: 'Ujian Semester', roles: ['admin', 'admin_akademik', 'guru'] },
                        ]
                    },
                    // Laporan Hafalan - dengan Harian, Mingguan
                    {
                        ...
                        roles: ['admin', 'admin_akademik', 'guru'],
                        children: [
                            { path: '/laporan/hafalan-harian', icon: Calendar, label: 'Harian', roles: ['admin', 'admin_akademik', 'guru'] },
                            { path: '/laporan/rekap-mingguan', icon: Calendar, label: 'Mingguan', roles: ['admin', 'admin_akademik', 'guru'] },
                        ]
                    },
                    // Laporan Akademik
                    {
                        ...
                        roles: ['admin', 'admin_akademik', 'guru'],
                        children: [
                            { path: '/laporan/akademik-santri', icon: Users, label: 'Raport', roles: ['admin', 'admin_akademik', 'guru'] },
                            { path: '/rekap-nilai/grafik', icon: Activity, label: 'Grafik Perkembangan', roles: ['admin', 'admin_akademik', 'guru'] },
                        ]
                    },
                ]
            },
...
const Sidebar = ({ mobileOpen, onClose }) => {
    ...
    useEffect(() => {
        ...
        const trySwitch = async (targetRole) => {
            if (activeRole !== targetRole && roles?.includes(targetRole)) {
                ...
                // Remove expensive invalidation on every navigation
            }
        }
        ...
        // 4. Akademik / Guru / Musyrif Context
            ...
            // Prioritize Admin Akademik, then Guru, then Musyrif
            if (roles?.includes('admin_akademik')) trySwitch('admin_akademik')
            else if (roles?.includes('guru')) trySwitch('guru')
        ...
        if (hasChildren) {
            return (
                <li key={item.id} className="mb-1">
                    ...
                    >
                        <item.icon size={20} className={`shrink-0 ${isOpen || isChildActive(item.children) ? 'text-[#BCF32F]' : 'text-gray-400'}`} />
    ...
    return (
        <>
            ...
            <aside
                className={`
                    fixed inset-y-0 left-0 w-[260px] bg-[#0A2619] border-r border-[#0A2619] 
                ...
                <div className="h-16 flex items-center px-6 border-b border-[#143d2a] bg-[#0A2619]">
                    <div className="flex items-center gap-3">
                        <img
                            ...
                            height="40"
                        />
                        <span className="text-lg font-bold text-white tracking-tight">SI-TAQUA</span>
                    </div>
                    <button
