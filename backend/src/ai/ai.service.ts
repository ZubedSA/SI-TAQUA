...
export class AiService {
  ...
  // Parse natural language to structured Intent/Entities
  async parseIntent(userPrompt: string, sender: string): Promise<any> {
    ...
    // 6. Detect Intent based on keywords
  ...
  // Generates natural language answer combining database result
  async generateResponse(userPrompt: string, intent: string, dbResult: any, sender: string): Promise<string> {
    ...
    // Random greeting helpers for human touch
    const greetings = [
    ...
    switch (intent) {
      ...
      case 'get_perkembangan_summary':
        ...
        if (dbResult.presensi && dbResult.presensi.length > 0) {
          ...
          const hadirDays = dbResult.presensi.filter((p: any) => p.status === 'hadir').length;
